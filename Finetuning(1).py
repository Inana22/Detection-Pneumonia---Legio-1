"""
================================================================================
EXPERIMENT B: PROPOSED MODEL (TWO-PHASE FINE TUNING)
================================================================================
"""
...
ROOT_EXP_DIR = 'Experiment_Proposed_FineTune'
...
# ==============================================================================
# 2. DATA LOADING & AUGMENTATION (DENGAN LAPORAN LENGKAP)
# ==============================================================================
print("\n" + "="*60)
print("[INIT] MENYIAPKAN AUGMENTASI MEDIS & DATASET")
print("="*60)
...
print("\n[INFO] STATUS AUGMENTASI AKTIF: (Tercatat)")
print("-" * 60)
...
history1 = model.fit(
    train_generator, steps_per_epoch=steps_per_epoch, validation_data=val_generator,
    epochs=EPOCHS_PHASE1, callbacks=[checkpoint], verbose=1
)
...
history2 = model.fit(
    train_generator, steps_per_epoch=steps_per_epoch, validation_data=val_generator,
    epochs=EPOCHS_PHASE1 + EPOCHS_PHASE2, initial_epoch=EPOCHS_PHASE1,
    callbacks=[checkpoint], verbose=1
)

final_model_path = os.path.join(MODEL_DIR, 'final_finetune_model.h5')
...
# A. Gabungkan History & Simpan JSON
h1 = history1.history; h2 = history2.history
history_combined = {
    'accuracy': h1['accuracy'] + h2['accuracy'],
    'val_accuracy': h1['val_accuracy'] + h2['val_accuracy'],
    'loss': h1['loss'] + h2['loss'],
    'val_loss': h1['val_loss'] + h2['val_loss'],
    'precision': h1['precision'] + h2['precision'],
    'val_precision': h1['val_precision'] + h2['val_precision'],
    'recall': h1['recall'] + h2['recall'],
    'val_recall': h1['val_recall'] + h2['val_recall']
}
...
def plot_metric_journal(train, val, name, filename):
    ...
    # Garis Merah Putus-putus Penanda Fine Tuning
    plt.axvline(x=EPOCHS_PHASE1, color='red', linestyle='--', alpha=0.6, label='Fine-Tuning Start')
    
    plt.title(f'Proposed Model: {name} History', fontsize=18, fontweight='bold', pad=20)
...
# C. Evaluasi Akhir (Best Model)
print("\n[PROCESS] Evaluating Best Fine-Tuned Model...")
...
plt.title('Proposed: Confusion Matrix', fontsize=16, fontweight='bold')
...
plt.title('Proposed: ROC Curve', fontsize=16, fontweight='bold')
...
print(f"\n[DONE] Hasil Fine-Tuning tersimpan di folder: {ROOT_EXP_DIR}")